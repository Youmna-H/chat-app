...
def get_tfidf_sim(prop_content_texts, num_responses, text):
	...
	sim_array.sort()
	most_similar =  sorted_most_similar[-args.num_responses:]
	return most_similar
...
if __name__ == "__main__":
	...
	if args.sim_model == 'word2vec':
		...
		if args.responses_per_stance == 0:
			vectors = get_vectors(prop_content_texts)
			most_similar = most_sim_cos(vectors, text, args.num_responses)
		else:
			vectors_pro = get_vectors(prop_content_texts_pro)
			most_similar_pro = most_sim_cos(vectors_pro, text, args.num_responses)
			vectors_con = get_vectors(prop_content_texts_con)
			most_similar_con = most_sim_cos(vectors_con, text, args.num_responses)
			vectors_neutral = get_vectors(prop_content_texts_neutral)
			most_similar_neutral = most_sim_cos(vectors_neutral, text, args.num_responses)
		...
		if args.responses_per_stance == 0:
			most_similar = get_tfidf_sim(prop_content_texts, args.num_responses, text)
		else:
			most_similar_pro = get_tfidf_sim(prop_content_texts_pro, args.num_responses, text)
			most_similar_con = get_tfidf_sim(prop_content_texts_con, args.num_responses, text)
			most_similar_neutral = get_tfidf_sim(prop_content_texts_neutral, args.num_responses, text)
