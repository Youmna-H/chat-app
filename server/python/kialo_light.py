...
def load(topic, model_name, sub_model_name="paraphrase-distilroberta-base-v1", is_indexed=False, **kwargs):
	...
	with open(config_path) as f:
		...
		for c in config["topics"]:
			if topic == c["id"]:
				...
				vec_path_all = pre_path  + c["sbert_path_all_original"]
	#read data files
	...
	if model_name == 'sbert':
		...
		if is_indexed == 1:
			...
			infile = open(vec_path_all,'rb')
			vecs_all = cPickle.load(infile)
			model = utils.get_sbert_model(sub_model_name)
	#return values
	#model is an object
	...
	return {"model": model, "vecs":vecs_all, "texts":texts_all, "stances":stances, "responses":responses, "lowercase_to_uppercase":lowercase_to_uppercase}
...
def get_suggested_responses(query, model_name, model, vecs, texts, stances, responses, lowercase_to_uppercase, num_responses=50, classify_responses=True, responses_to_response=False, is_indexed=False, **kwargs):
	pre_path = ""#"python/"
	# import time
	# start = time.time()
	config_path = pre_path + "kialo_config.json"
	distances, most_similar, sim_stances = [], [], []
	...
	if model_name == 'tfidf':
		...
		if model_name == 'sbert' and is_indexed: #TODO fix the indexed
			...
			if model_name == 'sbert' and False: #add check encoder check instead of false
				...
				distances, most_similar, sim_stances = utils.most_sim_cos(vecs, query_vec, stances, num_responses)
	...
	# sim_stances = stances[:num_responses]
		...
		for claim in most_similar:
			if claim in responses:
				for response in responses[claim]:
					...
					stances_responses.append(stances[r])
		most_similar = most_similar_response[:num_responses]
		sim_stances = stances_responses[:num_responses]
	
	#format return values
	return_array = []
	for i, text in enumerate(most_similar):
		return_array.append({"text": lowercase_to_uppercase[text], "stance": sim_stances[i]})
...
if __name__ == "__main__":
	...
	is_indexed = False
	num_responses = 50
	classify_responses = False
	responses_to_response = False
	topic = "veganism"

	loaded_values = load(topic, model_name, is_indexed=is_indexed)
	print("loaded")
	x = get_suggested_responses(query, model_name, loaded_values['model'], loaded_values['vecs'], loaded_values['texts'],
	loaded_values['stances'], loaded_values['responses'], loaded_values['lowercase_to_uppercase'],
